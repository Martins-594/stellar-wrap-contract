...
#[test]
fn test_admin_proposal_duration_overflow() {
    let env = Env::default();
    ...
    let result = env.try(|_| {
        client.create_admin_proposal(
...
#[test]
fn test_outbound_nonce_overflow() {
    let env = Env::default();
    let contract_id = env.register(StellarWrapContract, ());
    let client = StellarWrapContractClient::new(&env, &contract_id);
    ...
    // Set the outbound nonce to the maximum value to trigger overflow on increment
    env.as_contract(&contract_id, || {
        env.storage().set(&symbol_short!("nonce"), &u32::MAX);
        env.storage().set(&Symbol::new(&env, "outbound_nonce"), &u32::MAX);
    });

    let recipient = Address::generate(&env);
    let result = env.try(|_| {
        client.outbound(&recipient, &1i128);
        Ok(())
    });
